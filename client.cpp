...
{
    int cur = rq_cursor++;
    if(cur >= total_rq.size()){
       return;
    }
    string com = total_rq[cur].command;
    ...
    // are we set or get? this depends on the ratio
        ...
        // set command
        data_object *obj = m_obj_gen->get_object(obj_iter_type(m_config, 0),cur);
        unsigned int key_len;
        const char *key = obj->get_key(&key_len);
        //unsigned int value_len;
        //const char *value = obj->get_value(&value_len);
        unsigned int i;
        unsigned int value_len = total_rq[cur].value_size;
        string val="";
