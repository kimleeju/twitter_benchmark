...
rq* split(string str, char Delimiter, string com) {
    ...
    while(getline(iss, buffer, Delimiter)){
        ...
        _key_size = stoi(buffer);
        else if(cnt == 4)
        _value_size = stoi(buffer);
        else if(cnt ==6){
        _command = buffer;
        break;
        }    
    ...
    if(_command == com){
    //if(_command == "get" ){
        rq* req = new rq(_command, _key, _key_size, _value_size);
        return req;
    }
    else
        return NULL;
...
run_stats run_benchmark(int run_id, benchmark_config* cfg, object_generator* obj_gen)
{
    ...
    ifstream readFile;
    string trace = "/home/ljkim/cache-trace/samples/2020Mar/";
    
    string str;
    rq* req;
    
    readFile.open(trace+cfg->trace);
    if(readFile.is_open()){
        while(!readFile.eof()){
            getline(readFile, str);
            req=split(str,',',"set");
            if(req !=NULL)
                total_rq.push_back(*req);
        }
        readFile.close();
    }
    ...
    do {
        ...
        double progress = 0;
        if(cfg->requests)
            progress = 100.0 * total_ops / total_rq.size();
        else
