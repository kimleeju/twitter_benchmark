...
const char* object_generator::get_key(int iter, unsigned int *len, const int cur)
{
    unsigned int l;
    ...
    l = snprintf(m_key_buffer, sizeof(m_key_buffer)-1,
        "%s%llu", "hello_", m_key_index);
#endif
    l = snprintf(m_key_buffer, sizeof(m_key_buffer)-1,
        "%s", total_rq[cur].key.c_str());
    if (len != NULL) *len = l;

    return m_key_buffer;
}

data_object* object_generator::get_object(int iter, int cur)
{
    // compute key
    (void) get_key(iter, NULL, cur);

    // compute value
    unsigned int new_size = 0;
    get_value(m_key_index, &new_size);

    new_size = total_rq[cur].value_size;
    // compute expiry
